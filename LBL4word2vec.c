//log-bilinear model for word 2 vector
//modify from word2vec
//@author: luoqun9191@163.com
...
void InitNet()
{
    ...
    if (nce > 0)
    {
        a = posix_memalign((void **)&syn1nce, 128, (long long)(2 * window + 1)* layer1_size * sizeof(real));
        ...
        {  //train the cbow architecture
            if(nce > 0)
            {
                for (a = b; a < window * 2 + 1 - b; a++) if (a != window)
                {
                    ...
                    for (c = 0; c < layer1_size; c++) neu1[c] += syn1nce[a * layer1_size + c] * syn0[c + last_word * layer1_size];
                }
                for (d = 0; d < nce + 1; d++)
                {
                    if (d == 0)
                ...
                for (a = b; a < window * 2 + 1 - b; a++) if (a != window)
                {
                    ...
                    {
                        real temp = syn0[c + last_word * layer1_size];
                        syn0[c + last_word * layer1_size] += neu1e[c] * syn1nce[c + a * layer1_size];
                        syn1nce[c + a * layer1_size] += neu1e[c] * temp;
                    }
                }
            }
        }
        ...
        {  //train skip-gram
            if (nce > 0)
            {
                for (a = b; a < window * 2 + 1 - b; a++) if (a != window)
                {
                    ...
                    for (c = 0; c < layer1_size; c++) neu1[c] = syn1nce[a * layer1_size + c] * syn0[c + l1];
                    for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
                    for(d = 0; d < nce + 1; d++)
                    {
                        if (d == 0)
                    ...
                    {
                        real temp = syn0[c + l1];
                        syn0[c + l1] += neu1e[c] * syn1nce[a * layer1_size + c];
                        syn1nce[a * layer1_size + c] += neu1e[c] * temp;
                    }
                }
            }
        }
...
void TrainModel()
{
	if (nce <= 0)
	{
		printf("nce(noise example) must be larger than 0\n");
		exit(1);
	}
    long a, b, c, d;
    FILE *fo;
