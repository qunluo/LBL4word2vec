...
//my modify
int all_correct_total = 0;
int all_total = 0;
int all_correct_semantic = 0;
int all_semantic = 0;
int all_correct_syntactic = 0;
int all_syntactic = 0;
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    if ((!strcmp(st1, ":")) || (!strcmp(st1, "EXIT")) || feof(stdin)) {
      ...
      if (QID != 0) {
        ...
        all_correct_total += CACN;
        all_total += TACN;
        all_correct_semantic += SEAC;
        all_semantic += SECN;
        all_correct_syntactic += SYAC;
        all_syntactic += SYCN;
      }
      QID++;
      scanf("%s", st1);
  ...
  printf("%d %d %d %d %d %d\n",all_correct_total,all_total,all_correct_semantic,all_semantic,all_correct_syntactic,all_syntactic);
  return 0;
}
